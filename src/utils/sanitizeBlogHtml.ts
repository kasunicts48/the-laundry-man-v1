...
function extractBalancedDivInner(html: string, openPattern: RegExp): string | null {
...
function unwrapChatContainers(html: string): string {
...
function stripImportedAttributes(html: string): string {
  return html
    .replace(/\sclass="[^"]*"/gi, '')
    .replace(/\sstyle="[^"]*"/gi, '')
    .replace(/\sid="[^"]*"/gi, '')
...
function unwrapRedundantDivs(html: string): string {
  let result = html;
...
function cleanupImportedHtml(html: string): string {
  return html
    .replace(/<p>\s*&nbsp;\s*<\/p>/gi, '')
    .replace(/<p>\s*<\/p>/gi, '')
    .replace(/\n{3,}/g, '\n\n')
...
/** Remove ChatGPT / WordPress wrapper markup from imported blog HTML. */
